...
export class ThreadService {
  ...
  createThread(vectorStoreId: string): Observable<Thread> {
    const token = this.authService.getToken();
    return this.http.post<Thread>(`${this.apiUrl}/thread/${token}/`, { vector_store_id: vectorStoreId });
  }
  
  updateThread(id: string, vectorStoreId: string): Observable<Thread> {
    const token = this.authService.getToken();
    return this.http.put<Thread>(`${this.apiUrl}/thread/${token}/${id}/`, { vector_store_id: vectorStoreId });
  }
