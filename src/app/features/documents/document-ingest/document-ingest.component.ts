...
export class DocumentIngestComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    if (this.ingestMode === 'file' && this.selectedFile) {
      ingestObs = this.documentService.ingestDocument(this.vectorStoreId.trim(), this.selectedFile);
    } else if (this.ingestMode === 's3' && this.s3Url.trim()) {
      ingestObs = this.documentService.ingestDocument(this.vectorStoreId.trim(), undefined, this.s3Url.trim());
    } else {
    ...
    ingestObs.pipe(
      catchError(err => {
        ...
        this.error = `Failed to ingest document. ${err.error?.detail || err.message || 'Please try again.'}`;
      ...
      if (response) { // If response is not null (i.e., no error caught)
        ...
        setTimeout(() => {
            if (this.vectorStoreId) {
                ...
                this.router.navigate(['/documents'], { queryParams: { vectorStoreId: this.vectorStoreId } });
            } else {
  ...
  navigateBack(): void {
    if (this.vectorStoreId && this.route.snapshot.queryParamMap.has('vectorStoreId')) {
