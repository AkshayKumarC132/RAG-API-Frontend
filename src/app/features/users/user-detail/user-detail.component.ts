    ...
    <div class="container mx-auto p-4">
      <ng-container *ngIf="user$ | async as user; else loadingOrErrorState">
        <div *ngIf="user; else notFoundState">
          <h1 class="text-3xl font-bold mb-6">User Details: {{ user.username }}</h1>
          <div class="bg-white shadow-md rounded-lg px-8 pt-6 pb-8 mb-4">
            <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
              ...
              <div *ngIf="user.tenant_name">
                ...
                <dd class="mt-1 text-sm text-gray-900">{{ user.tenant_name }}</dd>
              </div>
              <div>
      ...
      <ng-template #loadingOrErrorState>
        ...
        <div *ngIf="error" class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-md shadow-md" role="alert">
          <strong class="font-bold">Error:</strong>
          <p>{{ error }}</p>
          <div class="mt-4">
            <a routerLink="/users" class="text-red-600 hover:text-red-800 font-semibold">Back to User List</a>
          </div>
        </div>
      </ng-template>
      ...
      <ng-template #notFoundState>
        ...
        <div *ngIf="!initialLoading && !error" class="text-center py-10">
          <h2 class="text-2xl font-semibold text-gray-700 mb-4">User Not Found</h2>
          <p class="text-gray-500 mb-6">The user you are looking for does not exist or could not be loaded.</p>
          <a routerLink="/users"
  ...
  ngOnInit(): void {
    this.user$ = this.route.paramMap.pipe(
      ...
      switchMap(params => {
        ...
        if (id) {
          return this.userService.getUser(id).pipe( // Assumes getUser from UserService handles API call
            catchError(err => {
              if (err.status === 404) {
