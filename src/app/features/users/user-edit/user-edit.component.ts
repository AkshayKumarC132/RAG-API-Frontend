...
export class UserEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.userForm = new FormGroup({
      ...
      role: new FormControl('', Validators.required), // Consider specific roles if applicable
    ...
    this.route.paramMap.pipe(
      ...
      switchMap(params => {
        ...
        if (id) {
          ...
          return this.userService.getUser(this.userId).pipe( // Assuming getUserById is named getUser
            catchError(err => {
              if (err.status === 404) {
      ...
      tap(user => { // This tap will only run if a user was fetched (edit mode)
      ...
      error: (err) => {
        ...
        console.error('Error in ngOnInit subscription for UserEditComponent:', err);
  ...
  onSubmit(): void {
    ...
    if (this.isEditMode) {
      if (formData.password) { // Only include password if provided in edit mode
    ...
    operation.pipe(
      ...
      error: (err) => {
        this.error = err.message || (this.isEditMode ? 'Failed to update user.' : 'Failed to create user.');
