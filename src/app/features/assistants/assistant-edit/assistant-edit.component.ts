  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.pipe(
      ...
      tap(assistant => {
        ...
        } else if (!this.error) { // If assistant is null but no specific error set from getAssistant
          this.error = 'Failed to load assistant data.';
        }
      }),
      catchError(err => {
        console.error('Error loading assistant for editing:', err);
        this.error = `Failed to load assistant data. ${err.error?.detail || ''}`;
        return of(null);
      }),
      finalize(() => {
  ...
  onSubmit(): void {
    ...
    this.assistantService.updateAssistant(
      ...
      catchError(err => {
        ...
        this.error = `Failed to update assistant. ${err.error?.detail || err.message || 'Please try again.'}`;
        return of(null);
      }),
      finalize(() => {
        this.isSaving = false;
