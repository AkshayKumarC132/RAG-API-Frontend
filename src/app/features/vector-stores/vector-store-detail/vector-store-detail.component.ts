  ...
  template: `
    <div class="container mx-auto p-4">
      ...
      <ng-template #loadingOrError>
        ...
        <div *ngIf="!error && !(vectorStore$ | async)" class="text-gray-700">
          <!-- Check async pipe directly for loading -->
          <p>Loading vector store details...</p>
        </div>
      </ng-template>

      <ng-template #notFound>
        <div *ngIf="!(vectorStore$ | async) && !error">
          <!-- Ensure this only shows if not loading and no error -->
          <h1 class="text-xl font-semibold mb-4">Vector Store Not Found</h1>
          <p class="text-gray-600 mb-4">
            The requested vector store could not be found.
          </p>
          <a
            routerLink="/vector-stores"
            class="text-blue-500 hover:text-blue-800"
          ...
          return this.vectorStoreService.getVectorStore(id).pipe(
            catchError((err) => {
              ...
              if (err.status === 404) {
                ...
                this.error =
                  err.message || "Failed to load vector store details.";
              }
